...
public class VectorIngestionService {
    ...
    public void embedAnnouncement(Announcement a) {
        try {
            ...
            ingestor.ingest(doc);
            log.info("✅ 공지 임베딩 저장 완료: {}", a.getUrl());

        } catch (Exception e) {
            log.error("❌ 공지 임베딩 실패", e);
    ...
    public void embedProgram(Program p) {
        try {
            ...
            metadata.put("url", p.getOriginalUrl());
            ...
            ingestor.ingest(doc);
            log.info("✅ 프로그램 임베딩 저장 완료: {}", p.getOriginalUrl());

        } catch (Exception e) {
            log.error("❌ 프로그램 임베딩 실패", e);
